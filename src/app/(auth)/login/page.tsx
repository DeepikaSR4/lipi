...
export default function LoginPage() {
  const { signInWithGoogle, signInWithEmail, error, loading, user } = useAuth();
  ...
  const handleEmail = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormError("");
    if (!email || !password) { setFormError("Please fill all fields."); return; }
  ...
  const handleGoogle = async () => {
    ...
    analytics.trackLoginCompleted("google");
    router.push("/dashboard");
  };
  ...
  return (
    <div className="min-h-screen bg-lipi-cream flex">
      ...
      <motion.div
        ...
        className="hidden md:flex flex-col justify-between bg-lipi-lavender border-r-2 border-lipi-border w-[42%] p-12"
      >
        <Link href="/" className="font-[family-name:var(--font-cormorant)] text-3xl font-bold">
          lipi
        </Link>
        <div>
          <h1 className="font-[family-name:var(--font-cormorant)] text-5xl font-semibold leading-tight mb-4">
            Welcome back,
            <br />
            <em>creator.</em>
          </h1>
          <p className="font-[family-name:var(--font-space-grotesk)] text-lipi-text/60 text-sm">
            Your handwriting fonts are waiting for you.
          </p>
        </div>
        <div className="font-[family-name:var(--font-caveat)] text-xl text-lipi-text/50 rotate-[-2deg]">
          "My font, my identity ✦"
        </div>
      </motion.div>
      ...
      >
        <div className="w-full max-w-sm">
          <div className="mb-8">
            <h2 className="font-[family-name:var(--font-space-grotesk)] text-2xl font-bold mb-1">Sign in</h2>
            <p className="text-sm text-lipi-muted font-[family-name:var(--font-space-grotesk)]">
              Don&apos;t have an account?{" "}
              <Link href="/signup" className="underline underline-offset-2 text-lipi-text font-semibold">
                Sign up
              </Link>
            </p>
          </div>
